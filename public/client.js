...
function setVolumeControl(volume) {
  ...
  const control = els.musicVolume.closest(".volume-control");
  els.musicVolume.value = volumeValue;
  els.musicVolume.style.setProperty("--volume-percent", `${volumeValue}%`);
  control.style.setProperty("--volume-percent", `${volumeValue}%`);
  control.style.setProperty("--volume-scale", volume);
}

function syncMusic(musicOn, musicVolume = 0.35) {
  els.musicButton.textContent = musicOn ? "Music on" : "Music off";
  els.backgroundMusic.volume = musicVolume;
  setVolumeControl(musicVolume);
  if (musicOn) {
    els.backgroundMusic.play().catch(() => {
      if (!els.gamePage.hidden) {
...
function renderKeyboard(game) {
  ...
  for (const letter of alphabet) {
    ...
    if (game.wrongGuesses.includes(letter)) {
...
function showRoundOverlay(round) {
  els.roundOverlay.textContent = `Round ${round}`;
  els.roundOverlay.classList.remove("show");
  void els.roundOverlay.offsetWidth;
  els.roundOverlay.classList.add("show");
}

function hideRoundOverlay() {
  els.roundOverlay.classList.remove("show");
  els.roundOverlay.textContent = "";
}
...
function render(game) {
  ...
  const previousRound = state.game ? state.game.round : game.round;
  const previousStatus = state.game ? state.game.status : game.status;
  state.game = game;
  ...
  const displayRound = game.status === "waiting" && !game.maskedWord ? 0 : game.round;
  els.roundTitle.textContent = `Round ${displayRound}`;
  if (hadGame && game.status === "playing" && (game.round > previousRound || previousStatus !== "playing")) {
    showRoundOverlay(game.round);
  ...
  syncMusic(game.musicOn, game.musicVolume);
...
function updateMusicVolume() {
  ...
  els.backgroundMusic.volume = volume;
...
async function endGames() {
  try {
    const data = await api("/api/end", { playerId: state.playerId });
    render(data.state);
  } catch (error) {
    els.message.textContent = error.message;
  }
...
els.endButton.addEventListener("click", endGames);
...
window.addEventListener("keydown", event => {
  ...
  if (me
    && /^[a-z]$/.test(letter)
    && !state.game.guesses.includes(letter)
    && !state.game.wrongGuesses.includes(letter)) {
    guess(letter);
  }
});
