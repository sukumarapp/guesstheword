...
function ensureHost() {
  if (hostId && players.get(hostId)?.online) return;
...
function publicState() {
  ...
  return {
    ...
    players: onlinePlayers().map(({ id, name, score, streak, online, misses = 0 }) => ({
      ...
      streak,
      misses,
      missesLeft: Math.max(0, MAX_MISSES - misses),
      online,
      isHost: id === hostId
    }))
  };
...
function handleGuess(playerId, letter) {
  ...
  game.lastGuess = { letter: guess, playerName: player.name, hit: game.word.includes(guess) };
...
async function handleApi(req, res) {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/join") {
      ...
      if (!player) {
        player = { id, name, score: 0, streak: 0, misses: 0, online: true };
      ...
      if (!hostId) hostId = id;
      broadcast();
      sendJson(res, 200, { ok: true, playerId: id, state: publicState() });
      return;
    }
